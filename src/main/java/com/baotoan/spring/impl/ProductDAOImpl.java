...
public class ProductDAOImpl extends BaseDAO implements ProductDAO {
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection connection) throws SQLException {
				PreparedStatement ps = connection.prepareStatement(sql,
						new String[] { product.getName(), String.valueOf(product.getOldPrice()), String.valueOf(product.getNewPrice()),
								String.valueOf(product.getViews()), String.valueOf(product.getReviews()),
								product.getTags(), String.valueOf(product.getPostId()), String.valueOf(product.getPromotionId()),
								String.valueOf(product.getImportDate()), String.valueOf(product.getCateId()), product.getDescription() });
				ps.setString(1, product.getName());
				ps.setInt(2, product.getOldPrice());
				ps.setInt(3, product.getNewPrice());
				ps.setInt(4, product.getViews());
				ps.setInt(5, product.getReviews());
				ps.setString(6, product.getTags());
				ps.setInt(7, product.getPostId());
				ps.setInt(8, product.getPromotionId());
				ps.setDate(9, new Date(product.getImportDate().getTime()));
				ps.setInt(10, product.getCateId());
	...
	public boolean updateProduct(Product product) {
		String sql = "update prods set name=?,old_price=?,new_price=?,views=?,reviews=?,tags=?,"
				+ "postId=?,promotionId=?,importDate=?,cateId=?,description=? WHERE id=?";
		return (jdbcTemplate.update(sql,
				new Object[] { product.getName(), product.getOldPrice(), product.getNewPrice(), product.getViews(),
						product.getReviews(), product.getTags(), product.getPostId(), product.getPromotionId(),
						product.getImportDate(), product.getCateId(), product.getDescription(), product.getId() }) > 0);
	}
	...
	public Map<String, Object> getProducts(Object key, int getBy, int sortBy, int numRecordPerPage, int currentPage) {
		...
		case Constant.GET_BY_BEST_SELL:
			sql = "SELECT prods.id, prods.`name`, prods.old_price, prods.new_price, prods.views, "
					...
					+ "prods.cateId, prods.description, SUM(order_detail.quantity) quantity"
		...
		case Constant.GET_BY_CATE:
			sql = "SELECT prods.id, prods.`name`, prods.old_price, prods.new_price, prods.views, "
					+ "prods.reviews, prods.tags, prods.postId, prods.promotionId, prods.importDate, "
					+ "prods.cateId, prods.description FROM prods INNER JOIN menu on menu.id=prods.cateId and menu.parentId="
					+ key + sort;
			break;
		case Constant.GET_BY_MANUFACTURER:
			sql = "SELECT prods.id, prods.`name`, prods.old_price, prods.new_price, prods.views, "
					+ "prods.reviews, prods.tags, prods.postId, prods.promotionId, prods.importDate, "
					+ "prods.cateId, prods.description FROM prods INNER JOIN menu on prods.cateId=" + key + sort;
			break;
		...
		case Constant.SEARCH_DEFAULT:
			sql = "SELECT * FROM prods WHERE name LIKE '%" + key + "%'" + spec + " or name LIKE '%" + key + "'" + spec
		...
		case Constant.SEARCH_ADVENCE:
			...
			MenuCate check = null;
			try {
				check = (MenuCate) jdbcTemplate.queryForObject("SELECT * FROM menu WHERE id=" + cateId,
						new MenuCateMapper());
			} catch (Exception e) {
				check = new MenuCate();
			...
			if (check.getParentId() == 0) {
				sql = "SELECT * from (select prods.id, prods.`name`, prods.old_price, prods.new_price, prods.views, "
						+ "prods.reviews, prods.tags, prods.postId, prods.promotionId, prods.importDate,"
						+ "prods.cateId, prods.description FROM prods INNER JOIN menu on prods.cateId = menu.id and parentId = "
						+ cateId + ") " + "as prods where prods.name like '%" + keyWord + "%' or prods.name like '%"
						+ keyWord + "' or prods.name like '" + keyWord + "%'" + sort;
			} else {
				sql = "SELECT * FROM prods WHERE name LIKE '%" + keyWord + "%' AND cateId=" + cateId
						+ " OR name LIKE '%" + keyWord + "' AND cateId=" + cateId + " OR name LIKE '" + keyWord
						+ "%' AND cateId=" + cateId + sort;
			}
			break;
		case Constant.DEFAULT:
			sql = "SELECT prods.id, prods.`name`, prods.old_price, prods.new_price, prods.views, "
		...
		String htmlForGrid = Pagination.generateHTML(totalRecordResult, numPageNeedShow, numRecordPerPage, currentPage,
				Pagination.GRID_PAGE);
		String htmlForToDoList = Pagination.generateHTML(totalRecordResult, numPageNeedShow, numRecordPerPage,
				currentPage, Pagination.TODO_LIST);
		...
		for (int i = 0; i < listResult.size(); i++) {
			...
			if(product.getListImage() != null && product.getListImage().size() > 0) {
