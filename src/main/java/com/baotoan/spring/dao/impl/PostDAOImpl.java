...
public class PostDAOImpl extends BaseDAO implements PostDAO {
	...
	public boolean addPost(Post post) {
		...
		return (jdbcTemplate.update(sql,
				new Object[] { post.getTitle(), post.getContent(), post.getPublishDate(), post.getAuthor() }) > 0);
	}

	public boolean updatePost(Post post) {
		String sql = "update posts set title=?,content=?,publishDate=?,author=? where id=?";
		return (jdbcTemplate.update(sql, new Object[] { post.getTitle(), post.getContent(), post.getPublishDate(),
				post.getAuthor(), post.getId() }) > 0);
	}
